...
const Testimonies = () => {
	...
	const TestimonyCard = ({ testimony, image, testifier }) => (
		<div className="p-4 sm:p-6 border-4 border-dashed border-[#FD9F2B] rounded-[10px] space-y-2 self-stretch">
			...
			<div className="flex gap-4 items-center">
				<div className="border-4 border-dashed border-[#FD9F2B] rounded-full aspect-square p-1 w-fit">
					<img
						src={image}
						alt={testifier}
						className="aspect-square rounded-full object-cover w-[44.77px] sm:w-[87.63px]"
					/>
				</div>
				<p className="text-sm sm:text-2xl">{testifier}</p>
			</div>
		</div>
	);
	...
	return (
		<section className="container mx-auto p-6 space-y-6 sm:mt-16 md:mt-24">
			<div className="space-y-6">
				<h3 className="font-satoshi text-2xl sm:text-[62.32px] max-w-[655px] mx-auto text-center uppercase">
			...
			<div className="flex justify-center">
				<Button
					...
					variant="solid"
					className="sm:w-[243px] sm:h-[58px] justify-center sm:text-lg sm:font-medium"
				/>
			</div>
		</section>
	);
