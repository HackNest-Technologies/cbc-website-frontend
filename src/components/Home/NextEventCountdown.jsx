...
const NextEventCountdown = () => {
  ...
  // Find the next upcoming OR ongoing event (consistent with UpcomingEvents)
  useLayoutEffect(() => {
    if (events && events.length > 0) {
      const now = new Date();
      
      // Find events that haven't ended yet (same logic as UpcomingEvents)
      const upcomingEvents = events
        .filter(event => {
          if (!event.end_date) return false;
          const eventEndDate = new Date(event.end_date);
          return eventEndDate >= now; // Events that end today or later
        })
        .sort((a, b) => {
          // Sort by which event ends soonest
      ...
      if (upcomingEvents.length > 0) {
        ...
        const nextEvent = upcomingEvents[0];
        setTargetDate(new Date(nextEvent.end_date)); // Count down to end date
      } else {
        // If no upcoming events, fallback to most recent event
        const sortedEvents = [...events].sort((a, b) => 
          new Date(b.end_date) - new Date(a.end_date)
        );
        setTargetDate(new Date(sortedEvents[0].end_date));
      }
    }
  }, [events]);
  ...
  // Find the event that matches the target date (end date)
  const currentEvent = events?.find(event => {
    const eventEndDate = new Date(event.end_date);
    return eventEndDate.getTime() === targetDate.getTime();
  });

  // If no exact match, find the event that ends soonest (from our filtered list)
  ...
  const displayEvent = getCurrentEvent();
  ...
  return (
    <section className="container mx-auto p6 py-12 lg:pt-[100px] xl:pt-[200px]">
      <div className="space-y-3">
        ...
        <p className="till leading-[100%] font-satoshi capitalize text-center text-xl md:text-[32px]">
          Till {displayEvent ? `${displayEvent.title} ends` : "the next event ends"}
        </p>
      </div>
    </section>
  );
