...
const StudyBtn = () => {
  ...
  const toggleBtn = (type) => setIsActive(type);
  const handleSwitchToBible = () => setIsActive("bible");

  return (
    <section className="container mx-auto">
      <section className="px-6 mb-10 mt-10 md:flex lg:mt-[120px] lg:mb-12 md:px-0 md:gap-4">
        {/* Calendar */}
        <Calendar
          selectedDate={selectedDate}
          isCalendarOpen={isCalendarOpen}
        ...
        {/* Toggle Buttons */}
        <div className="my-6 flex-1 md:my-0 flex justify-center">
          <div className="relative border rounded-[10.87px] p-[8.15px] flex gap-2 md:p-[15px] md:rounded-[20px]  lg:h-[80px] lg:w-[560px]">
            {["daily", "bible"].map((type) => {
              const isSelected = isActive === type;

              return (
                <div
                  key={type}
                  className="relative flex-1 cursor-pointer"
                  onClick={() => toggleBtn(type)}
                >
                  {/* Animated background */}
                  <AnimatePresence>
                    {isSelected && (
                      <motion.div
                        ...
                        className="absolute inset-0 bg-[#FD9F2B] rounded-[8px] md:rounded-[12px]"
                        transition={{
                          type: "spring",
                          stiffness: 400,
                          damping: 30,
      ...
      <section className="px-6 md:px-0">
        ...
        {!isLoading && !todayReading && (
          <p className="text-center py-8">
            No readings found for {formattedDate}.
          </p>
        )}
